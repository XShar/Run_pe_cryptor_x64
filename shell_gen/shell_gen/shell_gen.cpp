...
static void str_to_crypt (const char *str_to_crypt, const char *name_variable) 
{
	...
	//Криптование строки
	Xtea3_Data_Crypt(str, size_str, 1, (uint32_t*)&magic_key);
	...
	for (uint32_t j = 0; j < size_str; j++)
	{
		...
		else  fprintf(hFileHex, " 0x%02x, ", (((char*)str)[j]));
	};
...
static void bin_to_hex_gen(void)
{
	...
	//XTEA_encrypt((uint32_t*)(buffer + 8), size_file, pass, sizeof(pass));
	Xtea3_Data_Crypt((char*)(buffer + 8), size_file, 1, pass);
