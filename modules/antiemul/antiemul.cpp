...
uint8_t *antiemul_mem(uint32_t size_memory, uint8_t *data_protect, uint32_t size_data_protect) {
	...
	hash[0] = Murmur3(tmp_data, size_memory, 10);
	hash[1] = Murmur3(tmp_data, size_memory, 10);
	
	Xtea3_Data_Crypt((char*)(tmp_data), size_memory, 1, hash);
	Xtea3_Data_Crypt((char*)(tmp_data), size_memory, 0, hash);
...
void anti_emul_sleep(uintptr_t base, char *crypt_str, uint32_t size_str, uint32_t sleep_wait)
{
	...
	str_to_decrypt(crypt_str, size_str, magic_key, size_key);
